...
pub async fn discover(nodes: Arc<Nodes>) -> oneshot::Sender<()> {
    let own_ip = get_own_private_ip().unwrap_or("127.0.0.1".to_string());
    ...
    let broadcast_ip = format!(
        "{}.255",
        own_ip.split('.').take(3).collect::<Vec<&str>>().join(".")
    );
    ...
    loop {
        tokio::select! {
            ...
            _ = sleep(BROADCAST_INTERVAL) => {
                let message = own_ip.as_bytes();
                match socket
                    .send_to(message, (broadcast_ip.as_str(), BROADCAST_PORT))
                    .await
                {
                    Ok(_) => {
                    }
...
fn get_own_private_ip() -> Option<String> {
...
fn extract_private_ip(addr: &SocketAddr) -> Option<String> {
    match addr.ip() {
        IpAddr::V4(ipv4) => {
...
mod tests {
    ...
    fn test_nodes_add_and_test() {
        let nodes = Nodes::new();
        nodes.add("127.0.0.1".to_string());
        assert!(nodes.test("127.0.0.1"));
        assert!(!nodes.test("192.168.0.1"));
    }
    ...
    fn test_nodes_all() {
        let nodes = Nodes::new();
        nodes.add("127.0.0.1".to_string());
        nodes.add("192.168.0.1".to_string());
        let all_nodes = nodes.all();
        assert_eq!(all_nodes.len(), 2);
        assert!(all_nodes.iter().any(|node| node.ip == "127.0.0.1"));
        assert!(all_nodes.iter().any(|node| node.ip == "192.168.0.1"));
    }
